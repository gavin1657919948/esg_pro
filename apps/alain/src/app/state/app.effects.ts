...
export class AppEffects {
  ...
  loadModule$ = this.actions$.ofType('APP_LOAD_MODULE').do((action: any) => {
  ...
  @Effect({ dispatch: false })
  domainsAdd$ = this.actions$.ofType('APP_DOMAIN_ADD').do((action: any) => {
    return { type: 'AUTH_REALMS_ADD', payload: action.payload }
  })

  @Effect({ dispatch: false })
  domainSelect$ = this.actions$.ofType('APP_DOMAIN_SELECT').do((action: any) => {
  ...
  @Effect({ dispatch: false })
  redirectLogin: Observable<Action> = this.actions$.ofType(app.ActionTypes.APP_REDIRECT_LOGIN).do(() => {
